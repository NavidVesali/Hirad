...
async function fetchProductListData(categorySlug = "") {
    ...
    try {
        ...
        console.error("Error:", error);
        // Dispatch event with empty array to handle error state
        document.dispatchEvent(new CustomEvent("ProductListDataUpdated", {
            detail: { data: [], categorySlug }
        }));
    }
...
function updateProductList(eventData) {
    ...
    // Fade out dummy loaders or existing content
    const existingItems = container.querySelectorAll('.dummy-loader, .product-card');
    ...
    setTimeout(() => {
        ...
        // Check if the category slug matches and data exists
...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    if (storedData && storedData.length) {
        ...
        // Get default slug (tab 0) product count or show default loading
        ...
        if (storedProducts && Array.isArray(storedProducts)) {
            showDummyLoaders(Math.min(storedProducts.length, 5)); // Show loaders based on actual count, max 5
        } else {
            showDummyLoaders(3); // Show default 3 dummy loaders while fetching
        }

        changeTab(0);
    } else {
        fetchProductCatData();
        showDummyLoaders(3); // Show 3 dummy loaders while fetching
    }
});
